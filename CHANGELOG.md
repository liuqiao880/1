...
修复 AI 对话区域 6 项用户体验问题：

#### 已修复
- **FAB 菜单点击外部不收起**：添加 clickOutside 事件监听 + Esc 键关闭
- **移动端消息操作按钮不可见**：移除 hover 依赖，操作按钮始终显示
- **Provider 切换不自动填充预设**：切换 OpenAI/Gemini/Anthropic/Ollama 时自动填入对应 Base URL 和 Model
- **重置系统提示词无二次确认**：添加 confirm 确认对话框，防止误操作
- **语音输入覆盖已有内容**：改为追加模式，保留输入框原有文字
- **AI 设置页未保存返回无提示**：添加 isDirty 检测 + beforeunload 防护 + 离开确认
